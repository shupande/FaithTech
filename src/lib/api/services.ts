...
export const DownloadsAPI = {
  list: (params?: any) => 
    apiClient.get<PaginatedResponse<Download>>('/api/downloads', { params }),
  
  get: (id: string) => 
    apiClient.get<ApiResponse<Download>>(`/api/downloads/${id}`),
  
  create: (data: Omit<Download, 'id' | 'createdAt' | 'updatedAt' | 'downloads'>) => 
    apiClient.post<ApiResponse<Download>>('/api/downloads', data),
  
  update: (id: string, data: Partial<Download>) => 
    apiClient.put<ApiResponse<Download>>(`/api/downloads/${id}`, data),
  
  delete: (id: string) => 
    apiClient.delete<ApiResponse<void>>(`/api/downloads/${id}`),
  
  uploadFile: (file: File) => {
    const formData = new FormData()
    formData.append('file', file)
    return apiClient.post<ApiResponse<{ url: string }>>('/api/uploads/file', formData, {
      headers: {
...
export const MediaAPI = {
  // Assets
  listAssets: (params?: MediaSearchParams) => 
    apiClient.get<PaginatedResponse<MediaAsset>>('/api/media/assets', { params }),
  
  getAsset: (id: string) => 
    apiClient.get<ApiResponse<MediaAsset>>(`/api/media/assets/${id}`),
  
  createAsset: (data: FormData) => 
    apiClient.post<ApiResponse<MediaAsset>>('/api/media/assets', data, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    }),
  
  updateAsset: (id: string, data: UpdateMediaAssetDTO) => 
    apiClient.put<ApiResponse<MediaAsset>>(`/api/media/assets?id=${id}`, data),
  
  deleteAsset: (id: string) => 
    apiClient.delete<ApiResponse<void>>(`/api/media/assets?id=${id}`),

  // Versions
  createVersion: (data: FormData) => 
    apiClient.post<ApiResponse<MediaVersion>>('/api/media/versions', data, {
      headers: {
        'Content-Type': 'multipart/form-data',
      },
    }),
  ...
  // Categories
  listCategories: () => 
    apiClient.get<ApiResponse<MediaCategory[]>>('/api/media/categories'),
  
  createCategory: (data: CreateMediaCategoryDTO) => 
    apiClient.post<ApiResponse<MediaCategory>>('/api/media/categories', data),
  
  updateCategory: (id: string, data: UpdateMediaCategoryDTO) => 
    apiClient.put<ApiResponse<MediaCategory>>(`/api/media/categories?id=${id}`, data),
  
  deleteCategory: (id: string) => 
    apiClient.delete<ApiResponse<void>>(`/api/media/categories?id=${id}`),

  // Properties
  createProperty: (data: CreateMediaPropertyDTO) => 
    apiClient.post<ApiResponse<MediaProperty>>('/api/media/properties', data),
  
  deleteProperty: (id: string) => 
    apiClient.delete<ApiResponse<void>>(`/api/media/properties?id=${id}`),
} 